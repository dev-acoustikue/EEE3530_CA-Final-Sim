...
void q1_1_sim(int loops) {
	...
	for (int i = 0; i < loops; i++) {
		...
		if (cache[arr_idx] == ('b' + (4 * i) / 4)) { hits++; HITS; }
		else { cache[arr_idx] = ('b' + (4 * i) / 4); MISS; misses++; }

		// 2. access a[6 * i]
		arr_idx = (base_a + (6 * i) / 4) % CACHE_INDEX_SIZE;
		if (cache[arr_idx] == ('a' + (6 * i) / 4)) { hits++; HITS; }
		else { cache[arr_idx] = ('a' + (6 * i) / 4); MISS; misses++; }

		// 3. access b[i]
		arr_idx = (base_b + (i) / 4) % CACHE_INDEX_SIZE;
		if (cache[arr_idx] == ('b' + (i) / 4)) { hits++; HITS; }
		else { cache[arr_idx] = ('b' + (i) / 4); MISS; misses++; }

		// 4. access a[2000 - i]
		arr_idx = (base_a + (2000 - i) / 4) % CACHE_INDEX_SIZE;
		if (cache[arr_idx] == ('a' + (2000 - i) / 4)) { hits++; HITS; }
		else { cache[arr_idx] = ('a' + (2000 - i) / 4); MISS; misses++; }
...
void q1_2_sim(int loops) {
	...
	for (int i = 0; i < loops; i++) {
		...
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('b' + (4 * i) / 4)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 4) { cache[arr_idx].pop_front(); } // LRU
		...
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('a' + (6 * i) / 4)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 4) { cache[arr_idx].pop_front(); }
		...
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('b' + (i) / 4)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 4) { cache[arr_idx].pop_front(); }
		...
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('a' + (2000 - i) / 4)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 4) { cache[arr_idx].pop_front(); }
...
void q1_3_sim(int loops) {
	...
	for (int i = 0; i < loops; i++) {
		...
		if (cache[arr_idx] == ('b' + (4 * i) / 16)) { hits++; HITS; }
		else { cache[arr_idx] = ('b' + (4 * i) / 16); MISS; misses++; }

		// 2. access a[6 * i]
		arr_idx = (base_a + (6 * i) / 16) % CACHE_INDEX_SIZE;
		if (cache[arr_idx] == ('a' + (6 * i) / 16)) { hits++; HITS; }
		else { cache[arr_idx] = ('a' + (6 * i) / 16); MISS; misses++; }

		// 3. access b[i]
		arr_idx = (base_b + (i) / 16) % CACHE_INDEX_SIZE;
		if (cache[arr_idx] == ('b' + (i) / 16)) { hits++; HITS; }
		else { cache[arr_idx] = ('b' + (i) / 16); MISS; misses++; }

		// 4. access a[2000 - i]
		arr_idx = (base_a + (2000 - i) / 16) % CACHE_INDEX_SIZE;
		if (cache[arr_idx] == ('a' + (2000 - i) / 16)) { hits++; HITS; }
		else { cache[arr_idx] = ('a' + (2000 - i) / 16); MISS; misses++; }
...
void q1_4_sim(int loops) {
	...
	for (int i = 0; i < loops; i++) {
		...
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('b' + (4 * i) / 16)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 4) { cache[arr_idx].pop_front(); } // LRU
		...
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('a' + (6 * i) / 16)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 4) { cache[arr_idx].pop_front(); }
		...
		arr_idx = (base_b + (i) / 4) % index;
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('b' + (i) / 16)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 16) { cache[arr_idx].pop_front(); }
		...
		for (auto elm : cache[arr_idx]) { // find
			if (elm == ('a' + (2000 - i) / 16)) { HITS; hits++; break; } // when hit
			else { list_cnt++; }
		}
		if (list_cnt == cache[arr_idx].size()) { // When miss
			if (list_cnt == 4) { cache[arr_idx].pop_front(); }
